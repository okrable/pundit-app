...
export function useGameIdentity() {
  ...
  useEffect(() => {
    let active = true;
    void (async () => {
      const id = auth ?? (await getUserId());
      if (active) setResolved({ scope, id });
    })();
    return () => {
      active = false;
    };
...
export function useArchiveProgress(id: string | null) {
  ...
  const reload = useCallback(async () => {
    if (!id) return;
    try {
      const r = await archiveRepository.read(id);
      ...
      if (current.current === id)
        setError('Unable to read archive progress. Please retry.');
    }
  }, [id]);
  useEffect(() => {
    void reload();
  ...
  const update = useCallback(
    async (change: (r: ArchiveRecord) => ArchiveRecord) => {
      if (!id) throw Error('Archive is not ready');
      try {
        const r = await archiveRepository.update(id, change);
        if (current.current === id) {
          setRecord(r);
          setError(null);
        }
        ...
        if (current.current === id)
          setError(
            'Unable to save archive progress. Keep this screen open and retry.',
          );
        throw e;
      }
    },
  ...
  return {
    record: record?.userId === id ? record : null,
    error,
    reload,
    update,
  };
