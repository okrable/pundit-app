...
export interface StoredQuizResult {
  ...
  answers: boolean[];
  answer_details: QuizAnswerDetail[] | null;
}
...
export function serializeQuizResult(row: StoredQuizResult) {
  ...
  return {
    ...
    answers: row.answers.map((isCorrect, index) => ({
      ...details[index],
      isCorrect,
    })),
    statsPending: false,
...
export async function readQuizResult(
  client: PoolClient,
  userId: string,
  quizId: string,
) {
  const rows = await queryWithClient<StoredQuizResult>(
    client,
    `SELECT quiz_date::TEXT AS quiz_date, quiz_id, score, total_questions, answers, answer_details
     FROM results WHERE user_id=$1 AND quiz_id=$2`,
    [userId, quizId],
  );
  return rows[0] ?? null;
}

export async function withUserResultTransaction<T>(
  userId: string,
...
export async function recomputeUserQuizStats(
  client: PoolClient,
  userId: string,
) {
  const rows = await queryWithClient<{ score: number; answers: boolean[] }>(
    client,
    'SELECT score, answers FROM results WHERE user_id=$1',
    [userId],
  );
  const bestScore = rows.reduce(
    (best, row) => Math.max(best, Number(row.score)),
    0,
  );
  const totalCorrect = rows.reduce(
    (sum, row) => sum + row.answers.filter((v) => v === true).length,
    0,
  );
  await queryWithClient(
    client,
