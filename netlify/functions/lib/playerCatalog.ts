...
import {
  APPROVED_PLAYER_ALIASES,
  type SearchPlayer,
} from '../../../shared/playerSearch';
...
export async function getPlayerCatalog(): Promise<SearchPlayer[]> {
  ...
  const value = getBigQueryClient()
    .query({
      query: `SELECT player_id,player_name,date_of_birth,position FROM \`${projectId}.${dataset}.player_dic\``,
    ...
    .then(([rows]) =>
      rows
        ...
        .map((r: any) => ({
          ...
          birthYear: /\b(18|19|20)\d{2}\b/.exec(
            String(r.date_of_birth?.value ?? r.date_of_birth),
          )?.[0],
          position:
            typeof r.position === 'string' && r.position.length < 80
              ? r.position
              : undefined,
