...
import {
  legacyJourneyVisibility,
  type JourneyOutcome,
} from '../../../shared/journeyOutcome';
...
export async function readJourneyResult(
  client: PoolClient,
  userId: string,
  gameId: string,
) {
  const rows = await queryWithClient<JourneyRow>(
    client,
    `SELECT game_id,game_date::TEXT AS game_date,canonical_name,submitted_answer,outcome
     FROM career_game_results WHERE user_id=$1 AND game_id=$2`,
    [userId, gameId],
  );
  return rows[0] ?? null;
}
const defaults = {
  transaction: withUserResultTransaction,
  read: readJourneyResult,
  query: queryWithClient,
  game: getCareerGameForDate,
  today: getQuizDate,
};
export async function persistJourneyOutcome(
  ...
  return dependencies.transaction(userId, async (client) => {
    ...
    if (!row) {
      ...
      if (gameId !== `career-${dependencies.today()}`)
        throw new JourneyResultError(400, 'JOURNEY_EXPIRED');
      const game = await dependencies.game(date);
      if (!game) throw new JourneyResultError(503, 'JOURNEY_UNAVAILABLE');
      if (outcome === 'solved' && !matchesCareerAnswer(answer, game))
        throw new JourneyResultError(422, 'INCORRECT_ANSWER');
    ...
    if (!legacyJourneyVisibility(row.outcome, version))
      throw new JourneyResultError(409, 'JOURNEY_ALREADY_FINISHED');
