...
export function createArchiveHandler(
  ...
  return async (request: Request) => {
    ...
    if (!canAccessArchive())
      return reply({ error: 'Archive access unavailable' }, 403);
    const params = new URL(request.url).searchParams;
    const date = params.get('date');
    const before = params.get('before');
    const day = today();
    if (kind === 'quiz' && (!date || !isArchiveDate(date, day)))
      return reply({ error: 'Choose a past quiz date' }, 400);
    if (before && !isArchiveDate(before, day))
    ...
    try {
      ...
      if (kind === 'quiz') {
        ...
        return quiz
          ? reply({ quiz })
          : reply({ error: 'This quiz is not available' }, 404);
      }
      ...
      return reply({
        ...
        nextCursor: candidates.length > 20 ? page.at(-1)!.date : null,
      });
    } catch {
      return reply(
        { error: 'Archive temporarily unavailable. Please retry.' },
        503,
      );
    }
  };
