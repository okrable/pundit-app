...
import {
  readQuizResult,
  serializeQuizResult,
  recomputeUserQuizStats,
  withUserResultTransaction,
...
export function createSubmitQuizHandler(
  ...
  const handler: LambdaHandler = async (event) => {
    ...
    try {
      const { quizId, userId, answers, achievementSync } = JSON.parse(
        event.body || '{}',
      ) as SubmitQuizRequest;
      if (
        typeof quizId !== 'string' ||
        !/^quiz-\d{4}-\d{2}-\d{2}$/.test(quizId) ||
        typeof userId !== 'string' ||
        !userId
      )
        return reply(400, { error: 'Invalid quiz/user' });
      ...
      const limited = await enforceRateLimit(event, headers, {
        scope: 'submit-quiz',
        subject: userId,
        limit: 8,
        windowSeconds: 300,
      ...
      const scoreSubmission = async (): Promise<StoredQuizResult> => {
        const keys = await getAnswerKeyRows(
          date,
          'uk',
        ...
        return {
          ...
          score: details.reduce(
            (sum, a) =>
              sum + (a.isCorrect ? calculateQuizPoints(a.timeRemainingMs) : 0),
            0,
          ),
          answers: details.map((a) => a.isCorrect),
      ...
      if (isGuest) {
        ...
        return reply(200, {
          ...serializeQuizResult(row),
          streak: 1,
          bestScore: row.score,
        });
      }
      const response = await withUserResultTransaction(
        userId,
        async (client) => {
          ...
          if (!row) {
           ...
           VALUES($1,$2,$3,$4,$5,$6,$7::JSONB) ON CONFLICT(user_id,quiz_id) DO NOTHING RETURNING id`,
              [
                ...
                date,
                proposed.score,
                proposed.total_questions,
                proposed.answers,
                JSON.stringify(proposed.answer_details),
              ],
            );
          ...
          if (inserted) {
            const achievementEvent: DailyQuizAchievementEvent = {
              ...
              answersCorrect: row.answers,
              correctAtZero:
                row.answer_details?.some(
                  (a) => a.isCorrect && a.timeRemainingMs === 0,
                ) ?? false,
              allowCumulative: true,
            };
            const applied = await applyServerAchievementEvent(
              client,
              userId,
              achievementEvent,
              achievementSync,
            ...
            rejectedProposedIds = applied.rejectedProposedIds;
          } else {
            ...
            snapshot = await getServerAchievementSnapshotForUser(
              client,
              userId,
            );
          }
          return {
            ...
            rejectedAchievementIds: rejectedProposedIds,
          };
        },
      );
